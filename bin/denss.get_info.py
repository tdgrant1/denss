...
if __name__ == "__main__":
    ...
    print(" Modified Mean Density (voxels >0.01*max): %3.5f" % np.mean(rho[np.abs(rho)>0.01*rho.max()]))
    print(" Modified Std. Dev. of Density (voxels >0.01*max): %3.5f" % np.std(rho[np.abs(rho)>0.01*rho.max()]))
